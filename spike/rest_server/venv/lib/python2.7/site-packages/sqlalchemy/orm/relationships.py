...
class JoinCondition(object):
    ...
    def _annotate_present_fks(self):
        ...
        def visit_binary(binary):
            if not isinstance(binary.turn_left, sql.ColumnElement) or \
                    not isinstance(binary.turn_right, sql.ColumnElement):
                return

            if "foreign" not in binary.turn_left._annotations and \
                    "foreign" not in binary.turn_right._annotations:
                col = is_foreign(binary.turn_left, binary.turn_right)
                if col is not None:
                    if col.compare(binary.turn_left):
                        binary.turn_left = binary.turn_left._annotate(
                            {"foreign": True})
                    elif col.compare(binary.turn_right):
                        binary.turn_right = binary.turn_right._annotate(
                            {"foreign": True})
    ...
    def _refers_to_parent_table(self):
        ...
        def visit_binary(binary):
            c, f = binary.turn_left, binary.turn_right
            if (
    ...
    def _annotate_selfref(self, fn, remote_side_given):
        ...
        """
        def visit_binary(binary):
            equated = binary.turn_left.compare(binary.turn_right)
            if isinstance(binary.turn_left, expression.ColumnClause) and \
                    isinstance(binary.turn_right, expression.ColumnClause):
                # assume one to many - FKs are "remote"
                if fn(binary.turn_left):
                    binary.turn_left = binary.turn_left._annotate({"remote": True})
                if fn(binary.turn_right) and not equated:
                    binary.turn_right = binary.turn_right._annotate(
                        {"remote": True})
    ...
    def _annotate_remote_with_overlap(self):
        ...
        """
        def visit_binary(binary):
            binary.turn_left, binary.turn_right = proc_left_right(binary.turn_left,
                                                                  binary.turn_right)
            binary.turn_right, binary.turn_left = proc_left_right(binary.turn_right,
                                                                  binary.turn_left)
