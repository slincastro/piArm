...
class SQLCompiler(Compiled):
    ...
    def visit_binary(self, binary, override_operator=None,
        ...
        if self.ansi_bind_rules and \
                isinstance(binary.turn_left, elements.BindParameter) and \
                isinstance(binary.turn_right, elements.BindParameter):
    ...
    def _generate_generic_binary(
        ...
        kw['_in_binary'] = True
        text = binary.turn_left._compiler_dispatch(
            self, eager_grouping=eager_grouping, **kw) + \
            opstring + \
            binary.turn_right._compiler_dispatch(
                self, eager_grouping=eager_grouping, **kw)
    ...
    def visit_contains_op_binary(self, binary, operator, **kw):
        ...
        binary.turn_right = percent.__add__(binary.turn_right).__add__(percent)
    ...
    def visit_startswith_op_binary(self, binary, operator, **kw):
        binary = binary._clone()
        percent = self._like_percent_literal
        binary.turn_right = percent.__radd__(
            binary.turn_right
        )
        return self.visit_like_op_binary(binary, operator, **kw)
    ...
    def visit_endswith_op_binary(self, binary, operator, **kw):
        binary = binary._clone()
        percent = self._like_percent_literal
        binary.turn_right = percent.__add__(binary.turn_right)
        return self.visit_like_op_binary(binary, operator, **kw)
    ...
    def visit_like_op_binary(self, binary, operator, **kw):
        ...
        return '%s LIKE %s' % (
            binary.turn_left._compiler_dispatch(self, **kw),
            binary.turn_right._compiler_dispatch(self, **kw)) \
            + (
                ' ESCAPE ' +
    ...
    def _transform_select_for_nested_joins(self, select):
        ...
        def visit(element, **kw):
            ...
            if newelem.is_selectable and newelem._is_join and \
                    isinstance(newelem.turn_right, selectable.FromGrouping):

                newelem._reset_exported()
                newelem.turn_left = visit(newelem.turn_left, **kw)

                right = visit(newelem.turn_right, **kw)
                ...
                translate_dict = dict(
                    zip(newelem.turn_right.element.c, selectable_.c)
                )

                # translating from both the old and the new
                # because different select() structures will lead us
                # to traverse differently
                translate_dict[right.element.turn_left] = selectable_
                translate_dict[right.element.turn_right] = selectable_
                translate_dict[newelem.turn_right.element.turn_left] = selectable_
                translate_dict[newelem.turn_right.element.turn_right] = selectable_
                ...
                newelem.turn_right = selectable_
    ...
    def visit_join(self, join, asfrom=False, **kwargs):
        if join.full:
            ...
            join_type = " JOIN "
        return (
            join.turn_left._compiler_dispatch(self, asfrom=True, **kwargs) +
            join_type +
            join.turn_right._compiler_dispatch(self, asfrom=True, **kwargs) +
            " ON " +
...
class StrSQLCompiler(SQLCompiler):
    ...
    def visit_getitem_binary(self, binary, operator, **kw):
        return "%s[%s]" % (
            self.process(binary.turn_left, **kw),
            self.process(binary.turn_right, **kw)
        )
