...
def visit_binary_product(fn, expr):
    ...
    def visit(element):
        if isinstance(element, ScalarSelect):
            ...
            for l in visit(element.turn_left):
                for r in visit(element.turn_right):
                    fn(stack[0], l, r)
            stack.pop(0)
...
def adapt_criterion_to_null(crit, nulls):
    ...
    def visit_binary(binary):
        if isinstance(binary.turn_left, BindParameter) \
                and binary.turn_left._identifying_key in nulls:
            # reverse order if the NULL is on the left side
            binary.turn_left = binary.turn_right
            binary.turn_right = Null()
            binary.operator = operators.is_
            binary.negate = operators.isnot
        elif isinstance(binary.turn_right, BindParameter) \
                and binary.turn_right._identifying_key in nulls:
            binary.turn_right = Null()
            binary.operator = operators.is_
            binary.negate = operators.isnot
...
def splice_joins(left, right, stop_on=None):
    ...
    while stack:
        ...
        if isinstance(right, Join) and right is not stop_on:
            ...
            stack.append((right.turn_left, right))
        else:
    ...
    if clauses:
        def visit_binary(binary):
            if binary.operator == operators.eq:
                ...
                if binary.turn_left in cols and binary.turn_right in cols:
                    for c in reversed(columns):
                        if c.shares_lineage(binary.turn_right) and \
                            (not only_synonyms or
                             c.name == binary.turn_left.name):
                            omit.add(c)
                            break
...
def criterion_as_pairs(expression, consider_as_foreign_keys=None,
    ...
    def visit_binary(binary):
        if not any_operator and binary.operator is not operators.eq:
            return
        if not isinstance(binary.turn_left, ColumnElement) or \
                not isinstance(binary.turn_right, ColumnElement):
            return

        if consider_as_foreign_keys:
            if binary.turn_left in consider_as_foreign_keys and \
                (col_is(binary.turn_right, binary.turn_left) or
                 binary.turn_right not in consider_as_foreign_keys):
                pairs.append((binary.turn_right, binary.turn_left))
            elif binary.turn_right in consider_as_foreign_keys and \
                (col_is(binary.turn_left, binary.turn_right) or
                 binary.turn_left not in consider_as_foreign_keys):
                pairs.append((binary.turn_left, binary.turn_right))
        elif consider_as_referenced_keys:
            if binary.turn_left in consider_as_referenced_keys and \
                (col_is(binary.turn_right, binary.turn_left) or
                 binary.turn_right not in consider_as_referenced_keys):
                pairs.append((binary.turn_left, binary.turn_right))
            elif binary.turn_right in consider_as_referenced_keys and \
                (col_is(binary.turn_left, binary.turn_right) or
                 binary.turn_left not in consider_as_referenced_keys):
                pairs.append((binary.turn_right, binary.turn_left))
        else:
            if isinstance(binary.turn_left, Column) and \
                    isinstance(binary.turn_right, Column):
                if binary.turn_left.references(binary.turn_right):
                    pairs.append((binary.turn_right, binary.turn_left))
                elif binary.turn_right.references(binary.turn_left):
                    pairs.append((binary.turn_left, binary.turn_right))
    pairs = []
