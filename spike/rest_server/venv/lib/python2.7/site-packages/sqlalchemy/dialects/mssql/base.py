...
class MSSQLCompiler(compiler.SQLCompiler):
    ...
    def visit_binary(self, binary, **kwargs):
        ...
        """
        if (
            isinstance(binary.turn_left, expression.BindParameter)
            and binary.operator == operator.eq
            and not isinstance(binary.turn_right, expression.BindParameter)
        ):
            return self.process(
                expression.BinaryExpression(binary.turn_right,
                                            binary.turn_left,
                                            binary.operator),
                **kwargs)
