...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def visit_concat_op_binary(self, binary, operator, **kw):
        return "concat(%s, %s)" % (self.process(binary.turn_left),
                                   self.process(binary.turn_right))

    def visit_match_op_binary(self, binary, operator, **kw):
        return "MATCH (%s) AGAINST (%s IN BOOLEAN MODE)" % \
            (self.process(binary.turn_left), self.process(binary.turn_right))
    ...
    def visit_join(self, join, asfrom=False, **kwargs):
        ...
        return ''.join(
            (self.process(join.turn_left, asfrom=True, **kwargs),
             join_type,
             self.process(join.turn_right, asfrom=True, **kwargs),
             " ON ",
