...
class OracleCompiler(compiler.SQLCompiler):
    ...
    def visit_mod_binary(self, binary, operator, **kw):
        return "mod(%s, %s)" % (self.process(binary.turn_left, **kw),
    ...
    def visit_match_op_binary(self, binary, operator, **kw):
        return "CONTAINS (%s, %s)" % (self.process(binary.turn_left),
                                      self.process(binary.turn_right))
    ...
    def visit_join(self, join, **kwargs):
        if self.dialect.use_ansi:
            ...
            kwargs['asfrom'] = True
            if isinstance(join.turn_right, expression.FromGrouping):
                right = join.turn_right.element
            else:
                right = join.turn_right
            return self.process(join.turn_left, **kwargs) + \
            ...
            if join.isouter:
                def visit_binary(binary):
                    if binary.operator == sql_operators.eq:
                        if join.turn_right.is_derived_from(binary.turn_left.table):
                            binary.turn_left = _OuterJoinColumn(binary.turn_left)
                        elif join.turn_right.is_derived_from(binary.turn_right.table):
                            binary.turn_right = _OuterJoinColumn(binary.turn_right)
                clauses.append(visitors.cloned_traverse(
                    join.onclause, {}, {'binary': visit_binary}))
            else:
                clauses.append(join.onclause)

            for j in join.turn_left, join.turn_right:
